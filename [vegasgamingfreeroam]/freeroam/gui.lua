...
local classInfo = {
	wnd = {className = 'Window', padding = {45, 10, 10, 10}, isContainer = true},
	...
	chk = {className = 'CheckBox', height = 20, padding = {0, 10}},
...
function _buildWindow(wnd, baseWnd, parentWnd)
	...
	if wndClass == 'wnd' then
		...
		guiWindowSetMovable(elem,false)
		--guiWindowSetSizable(elem, false)
		--guiBringToFront(elem)
		g_openedWindows[wnd] = true
	elseif wndClass == 'btn' then
		elem = guiCreateButton(relX, relY, relWidth, relHeight, wnd.text or wnd.id or '', true, parentWnd.element,wnd.tip,wnd.customrenk or true)
